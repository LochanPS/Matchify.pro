...
async function generateCodesForAllUsers() {
  try {
    console.log('🔍 Checking for users without codes...\n');
    ...
    for (const user of usersWithoutCodes) {
      ...
      if (!user.playerCode) {
        ...
        playerCodesAdded++;
        console.log(`✅ Player Code: ${updates.playerCode} → ${user.name} (${user.email})`);
      }
      ...
      if (!user.umpireCode) {
        ...
        umpireCodesAdded++;
        console.log(`✅ Umpire Code: ${updates.umpireCode} → ${user.name} (${user.email})`);
      }
