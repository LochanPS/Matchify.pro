...
function AppContent() {
  ...
  // ── Silent keep-warm ping ────────────────────────────────────────────────
  // Pings /api/health on mount (wakes the backend immediately) then every
  // 7 minutes so it never goes cold while the app is open.
  // Fully silent — no UI shown, no errors surfaced, no blocking.
  useEffect(() => {
    let alive = true;
    const ping = () =>
      api.get('/health', { _skipLogout: true, _noRetry: true, timeout: 20000 })
        .catch(() => {}); // always silent

    ping(); // fire immediately on app open to wake backend
