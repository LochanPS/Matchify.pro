...
export default function FloatingWhatsApp() {
  ...
  // Dismissed only for THIS app session — sessionStorage clears when the tab/PWA
  // is fully closed, so the button (with its label) returns on the next cold open.
  const [dismissed, setDismissed] = useState(() => {
    try { return sessionStorage.getItem('_waDismissed') === '1'; } catch { return false; }
  });
  ...
  const dismiss = (e) => {
    e.preventDefault();
    e.stopPropagation();
    try { sessionStorage.setItem('_waDismissed', '1'); } catch { /* ignore */ }
    setDismissed(true);
  };

  // Hidden for this session, or not a main page.
  ...
  return (
    <div
      style={{
        position: 'fixed',
        // Hug the 480px app column's right edge on wide screens; 16px in on phones.
    ...
    >
      ...
      {/* Small × — hide for this session (returns on next app open) */}
      <button
        onClick={dismiss}
