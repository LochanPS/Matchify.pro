  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center z-[9999] p-4"
      ...
      <div className="relative w-full max-w-sm rounded-3xl overflow-hidden shadow-2xl"
        style={{ background: F.card, border: `1px solid rgba(245,158,11,0.15)` }}>

        {/* Top gradient line */}
        <div className="h-1" style={{ background: 'linear-gradient(90deg,#F59E0B,#FCD34D,#a855f7)' }} />
            ...
            <>
              ...
              <div className="text-center mb-4">
                <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full mb-2 text-xs font-bold"
                  style={{ background: 'rgba(245,158,11,0.1)', border: '1px solid rgba(245,158,11,0.2)', color: F.green }}>
                ...
                {[
                  ...
                  <button key={tab.key} type="button"
                    ...
                    style={{
                      ...
                      boxShadow: photoTab === tab.key ? '0 2px 8px rgba(245,158,11,0.25)' : 'none',
                    }}>
                    {tab.icon} {tab.label}
                  </button>
                ))}
                ...
                <>
                  <div
                    ...
                    style={{
                      ...
                      border: `2px dashed ${preview && !selectedAvatar ? 'rgba(245,158,11,0.4)' : 'rgba(245,158,11,0.2)'}`,
                      background: preview && !selectedAvatar ? 'transparent' : 'rgba(245,158,11,0.02)',
                    }}>
                    {preview && !selectedAvatar ? (
                      ...
                      <div className="flex flex-col items-center justify-center h-full gap-2 transition-all group-hover:scale-105">
                        <div className="w-10 h-10 rounded-2xl flex items-center justify-center"
                          style={{ background: 'linear-gradient(135deg,rgba(245,158,11,0.15),rgba(245,158,11,0.1))' }}>
                    ...
                    {AVATARS.map((url, i) => {
                      ...
                      return (
                        <button key={i} type="button" onClick={() => handleSelectAvatar(url)}
                          ...
                          style={{
                            aspectRatio: '1',
                            border: isSelected ? '2.5px solid #F59E0B' : '2px solid rgba(255,255,255,0.1)',
                            boxShadow: isSelected ? '0 0 20px rgba(245,158,11,0.45)' : '0 2px 8px rgba(0,0,0,0.3)',
              ...
              {(() => {
                ...
                return (
                  <button onClick={handleUploadPhoto} disabled={!ready || uploading}
                    ...
                    style={{
                      background: ready && !uploading ? 'linear-gradient(135deg,#F59E0B,#D97706)' : 'rgba(255,255,255,0.06)',
                      color: ready && !uploading ? '#003320' : F.dim,
                      cursor: !ready || uploading ? 'not-allowed' : 'pointer',
                      boxShadow: ready && !uploading ? '0 8px 20px rgba(245,158,11,0.25)' : 'none',
                    }}>
                    {uploading ? (
                ...
                <Field label="State" required>
                  <div ref={stateRef} style={{ position: 'relative' }}>
                    <input type="text" value={stateInput}
                      ...
                      style={{ ...inputStyle, borderColor: form.state ? 'rgba(245,158,11,0.3)' : F.border }}
                      autoComplete="off" />
                    {form.state && (
                  ...
                  <div className="flex gap-2">
                    {[['MALE','♂ Male'],['FEMALE','♀ Female'],['OTHER','⚧ Other']].map(([val, label]) => (
                      <button key={val} type="button"
                        ...
                        style={{
                          background: form.gender === val ? 'rgba(245,158,11,0.15)' : 'rgba(255,255,255,0.04)',
                          border: `1px solid ${form.gender === val ? 'rgba(245,158,11,0.4)' : F.border}`,
