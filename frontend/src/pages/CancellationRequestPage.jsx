...
const getImageUrl = (url) => {
  ...
  if (url.startsWith('/uploads')) {
    const baseUrl = import.meta.env.VITE_API_URL?.replace('/api', '') || 'https://matchify-backend-production-cb58.up.railway.app';
    return `${baseUrl}${url}`;
  }
  return url;
};
  ...
  return (
    <div className="min-h-screen relative" style={{ background: BG }}>
      ...
      <div className="relative px-4 py-5 space-y-4">
        ...
        <div style={CARD} className="overflow-hidden">
          <div className="px-4 py-3" style={{ borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
            <p className="text-xs font-black uppercase tracking-wider flex items-center gap-1.5" style={{ color: 'rgba(245,158,11,0.8)' }}>
          ...
          <div className="p-4 space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="rounded-xl p-3" style={{ background: 'rgba(245,158,11,0.1)', border: '1px solid rgba(245,158,11,0.25)' }}>
                <p className="text-xs mb-1" style={{ color: 'rgba(245,158,11,0.7)' }}>Refund Amount</p>
                <p className="text-xl font-black" style={{ color: '#F59E0B' }}>₹{registration.refundAmount || registration.amountTotal}</p>
              </div>
              <div className="rounded-xl p-3" style={{ background: 'rgba(245,158,11,0.1)', border: '1px solid rgba(245,158,11,0.25)' }}>
                <p className="text-xs mb-1" style={{ color: 'rgba(245,158,11,0.7)' }}>UPI ID</p>
        ...
        {registration.status === 'cancelled' && (
          <div className="rounded-2xl p-5 text-center" style={{ background: 'rgba(245,158,11,0.1)', border: '1px solid rgba(245,158,11,0.3)' }}>
            <Check className="w-10 h-10 mx-auto mb-2" style={{ color: '#F59E0B' }} />
      ...
      {resultModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(0,0,0,0.75)', backdropFilter: 'blur(8px)' }}>
          <div className="w-full rounded-2xl overflow-hidden" style={{
            ...
            border: `2px solid ${resultModal.type === 'success' ? 'rgba(245,158,11,0.4)' : 'rgba(239,68,68,0.4)'}`,
          }}>
            <div className="p-5 text-center">
              <div className="w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4" style={{ background: resultModal.type === 'success' ? 'rgba(245,158,11,0.15)' : 'rgba(239,68,68,0.15)' }}>
              ...
              <button
                ...
                style={{
                  background: resultModal.type === 'success' ? 'linear-gradient(135deg,#F59E0B,#FCD34D)' : 'linear-gradient(135deg,#ef4444,#dc2626)',
