...
const KnockoutDisplay = ({ data, matches, user, isOrganizer, onAssignUmpire, onViewMatchDetails, categoryFormat }) => {
                ...
                <g key={`${pos.roundIndex}-${pos.matchIndex}`} transform={`translate(${x}, ${y})`}>
                  ...
                  <g transform="translate(0, 60)">
                    ...
                    {categoryFormat === 'doubles' ? (
                      <>
                        <text x={cardWidth / 2} y="20" textAnchor="middle" fill={match.winner === 1 || dbMatch?.winnerId === player1?.id ? '#ffffff' : '#d1d5db'} fontSize="13" fontWeight="700">
                          {(player1?.name || 'TBD').substring(0, 24)}
                        </text>
                      </>
                    ) : (
                      <text x="12" y="20" fill={match.winner === 1 || dbMatch?.winnerId === player1?.id ? '#ffffff' : '#d1d5db'} fontSize="14" fontWeight="700">
                        {(player1?.name || 'TBD').substring(0, 22)}
                      </text>
                    )}
                    {/* Player 1 Detailed Set Scores */}
                    {isCompleted && dbMatch?.score && (
                      <text x={categoryFormat === 'doubles' ? cardWidth / 2 : 12} y="35" textAnchor={categoryFormat === 'doubles' ? 'middle' : 'start'} fill="#9ca3af" fontSize="10" fontWeight="500">
                  ...
                  {/* Divider - Show VS for doubles, line for singles */}
                  {categoryFormat === 'doubles' ? (
                    <g transform="translate(0, 110)">
                      <rect x={cardWidth / 2 - 25} y="-12" width="50" height="24" rx="12" fill="rgba(30, 41, 59, 0.9)" stroke="rgba(168, 85, 247, 0.3)" strokeWidth="1" />
                      <text x={cardWidth / 2} y="5" textAnchor="middle" fill="#a855f7" fontSize="11" fontWeight="700">VS</text>
                    </g>
                  ) : (
                  ...
                  <g transform="translate(0, 110)">
                    ...
                    {categoryFormat === 'doubles' ? (
                      <>
                        <text x={cardWidth / 2} y="20" textAnchor="middle" fill={match.winner === 2 || dbMatch?.winnerId === player2?.id ? '#ffffff' : '#d1d5db'} fontSize="13" fontWeight="700">
                          {(player2?.name || 'TBD').substring(0, 24)}
                        </text>
                      </>
                    ) : (
                      <text x="12" y="20" fill={match.winner === 2 || dbMatch?.winnerId === player2?.id ? '#ffffff' : '#d1d5db'} fontSize="14" fontWeight="700">
