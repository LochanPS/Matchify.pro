...
const STEPS = [
  { num: '01', title: 'Create your tournament',  desc: 'Set format, categories, prize money, and payment in under 5 minutes.', color: '#F59E0B' },
  { num: '02', title: 'Players register online', desc: 'Share one link. Players sign up and pay via UPI — fully automated.',   color: '#FCD34D' },
...
const FEATURES = [
  ...
  { Icon: SignalIcon,        title: 'Live point-by-point scoring',  desc: 'Umpires score from the court. Spectators watch every point in real time.', tag: 'WebSocket live',  color: '#F59E0B', bg: 'rgba(245,158,11,0.1)' },
  { Icon: CurrencyRupeeIcon, title: 'UPI payment collection',       desc: 'Players pay via QR. You verify. Wallet refunds processed instantly.',      tag: '100% secure',     color: '#F59E0B', bg: 'rgba(245,158,11,0.1)' },
  { Icon: UserGroupIcon,     title: 'Doubles partner network',      desc: 'Players invite partners in-app. Doubles pairs registered automatically.',  tag: "Men's·Women's·Mixed", color: '#F59E0B', bg: 'rgba(245,158,11,0.1)' },
...
const TESTIMONIALS = [
  { name: 'Arjun Kumar',  role: 'Tournament Organizer', city: 'Delhi',     badge: 'Organizer ⚙️',         badgeColor: '#FCD34D', text: 'Automated draws and payment verification save me 4+ hours per event. What used to take a full day now takes 30 minutes.' },
...
const HP_CSS = `
  ...
  .hp-shim {
    background: linear-gradient(90deg,#F59E0B 0%,#FCD34D 40%,#a855f7 70%,#F59E0B 100%);
  ...
  .hp-dot { width:8px;height:8px;border-radius:50%;background:#F59E0B;box-shadow:0 0 7px #F59E0B; animation:hp-dot 1.4s ease-in-out infinite; }
...
function DashboardMockup() {
  return (
    <div className="relative w-full rounded-2xl overflow-hidden"
      style={{ background:'#0d1025', border:'1px solid rgba(245,158,11,0.2)', boxShadow:'0 32px 80px rgba(0,0,0,.7), 0 0 0 1px rgba(255,255,255,.04)', maxWidth:440 }}>
      ...
      <div className="p-4 space-y-3">
        ...
        <div className="flex items-start justify-between gap-2">
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-xs font-bold" style={{ color:'#F59E0B' }}>LIVE</span>
          ...
          <div className="text-right flex-shrink-0">
            ...
            <p className="text-base font-black" style={{ color:'#F59E0B' }}>₹50,000</p>
          </div>
        </div>

        {/* Live match card */}
        <div className="rounded-xl p-3" style={{ background:'rgba(245,158,11,.05)', border:'1px solid rgba(245,158,11,.18)' }}>
          <div className="flex items-center justify-between mb-2.5">
            ...
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-xs font-bold" style={{ color:'#F59E0B' }}>In Progress</span>
            </div>
          </div>
            ...
            {[
              ...
              <div key={i} className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  {p.serving && <div className="w-1.5 h-1.5 rounded-full" style={{ background:'#FCD34D' }} />}
                ...
                <span className="text-base font-black" style={{ color: p.winning ? '#F59E0B' : 'rgba(255,255,255,.5)' }}>{p.score}</span>
              </div>
            ))}
          </div>
          <div className="mt-2.5 rounded-full overflow-hidden" style={{ height:3, background:'rgba(255,255,255,.06)' }}>
            <div className="hp-score-bar" style={{ width:'54.5%', height:'100%', background:'linear-gradient(90deg,#F59E0B,#FCD34D)' }} />
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-2">
          {[
            { label:'Registered', val:'24 / 32', color:'#FCD34D' },
            { label:'Completed',  val:'12 matches', color:'#F59E0B' },
    ...
    <div className="min-h-screen overflow-x-hidden" style={{ background:'#07071a', color:'white' }}>
      ...
      <div className="absolute inset-0 z-0 pointer-events-none overflow-hidden" aria-hidden>
        ...
        <div className="hp-glow hp-float absolute -top-32 -right-32 w-[500px] h-[500px] rounded-full"
          style={{ background:'radial-gradient(circle, rgba(245,158,11,.1) 0%, transparent 65%)', filter:'blur(1px)' }} />
        ...
        <div className="hp-glow absolute bottom-0 right-0 w-[450px] h-[450px] rounded-full"
          style={{ background:'radial-gradient(circle, rgba(245,158,11,.06) 0%, transparent 65%)', animationDelay:'1s' }} />
      </div>
          ...
          {user ? (
            <div className="hp-fade d4 flex flex-col gap-3 mb-8">
              <Link to={getDashboardLink()}
                ...
                style={{ background:'linear-gradient(135deg,#D97706,#F59E0B)', color:'#050810', boxShadow:'0 4px 24px rgba(245,158,11,.4)' }}>
                Go to Dashboard
            ...
            <div className="hp-fade d4 flex flex-col gap-3 mb-8">
              ...
              <p className="text-center text-xs" style={{ color:'rgba(255,255,255,.35)' }}>
                Already organizing?{' '}
                <Link to="/login" style={{ color:'#F59E0B' }} className="font-semibold">Sign In</Link>
              </p>
            </div>
          )}
              ...
              <div className="flex -space-x-2">
                {['R','P','A','K'].map((l, i) => (
                  <div key={i} className="w-7 h-7 rounded-full border-2 flex items-center justify-center text-xs font-black"
                    style={{ background:'linear-gradient(135deg,#D97706,#F59E0B)', borderColor:'#07071a', color:'#050810' }}>{l}</div>
                ))}
              </div>
            ...
            <div className="grid grid-cols-4 gap-2">
              {stats.map((s, i) => (
                <div key={i} className="flex flex-col items-center gap-1 py-4 px-2 rounded-2xl"
                  ...
                  <span className="text-lg font-black" style={{ color:'#F59E0B' }}>{s.value}</span>
      ...
      <section className="relative z-10 py-12 px-5">
        <div className="mx-auto" style={{ maxWidth:460 }}>
          <Reveal className="text-center mb-8">
            <span className="inline-block px-4 py-1.5 rounded-full text-xs font-black tracking-wider uppercase mb-3"
              style={{ background:'rgba(245,158,11,.08)', color:'#FCD34D', border:'1px solid rgba(245,158,11,.2)' }}>
              🎯 How It Works
            </span>
            <h2 className="text-2xl font-black leading-tight">
              From idea to live tournament<br />
              <span style={{ color:'#FCD34D' }}>in 3 simple steps.</span>
            </h2>
          </Reveal>
          ...
          <div className="relative flex flex-col gap-0">
            ...
            <div className="absolute left-[27px] top-10 bottom-10 w-0.5"
              style={{ background:'linear-gradient(180deg,#F59E0B,#FCD34D,#a855f7)', opacity:.35 }} aria-hidden />
          ...
          {!user && (
            <Reveal delay={0.4} className="mt-2">
              <Link to="/register"
                ...
                style={{ background:'linear-gradient(135deg,#D97706,#F59E0B)', color:'#050810', boxShadow:'0 4px 20px rgba(245,158,11,.35)' }}>
        ...
        <div className="mx-auto" style={{ maxWidth:460 }}>
          <Reveal className="text-center mb-8">
            ...
            <h2 className="text-2xl font-black leading-tight">
              ...
              <span style={{ color:'#F59E0B' }}>Nothing you don't.</span>
            </h2>
                    ...
                    <div className="p-3 space-y-2">
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-1">
                          <div className="hp-dot" style={{ width:6, height:6 }} />
                          <span style={{ color:'#F59E0B', fontSize:10, fontWeight:700 }}>LIVE</span>
                        </div>
                      </div>
                      {/* Big score display */}
                      <div className="rounded-xl p-3" style={{ background:'rgba(245,158,11,.07)', border:'1px solid rgba(245,158,11,.2)' }}>
                        ...
                        <div className="flex items-end justify-between">
                          <div>
                            ...
                            <p className="font-black" style={{ color:'#F59E0B', fontSize:28, lineHeight:1 }}>19</p>
                          </div>
                      ...
                      <div className="rounded-xl p-2.5" style={{ background:'rgba(255,255,255,.03)', border:'1px solid rgba(255,255,255,.06)' }}>
                        ...
                        <div className="flex gap-2">
                          <span style={{ background:'rgba(245,158,11,.12)', color:'#F59E0B', fontSize:10, fontWeight:700 }} className="px-2 py-0.5 rounded-lg">21–18</span>
        ...
        <div className="mx-auto" style={{ maxWidth:460 }}>
          <Reveal>
            <div className="relative rounded-3xl overflow-hidden text-center p-8"
              style={{ background:'linear-gradient(135deg,rgba(245,158,11,.07),rgba(245,158,11,.07))', border:'1px solid rgba(245,158,11,.2)' }}>
              <div className="absolute inset-0 pointer-events-none"
                style={{ background:'radial-gradient(ellipse at 50% 0%,rgba(245,158,11,.1) 0%,transparent 65%)' }} aria-hidden />
              <div className="relative">
                <div className="inline-flex items-center justify-center w-16 h-16 rounded-2xl mb-5 text-3xl"
                  style={{ background:'linear-gradient(135deg,#D97706,#FCD34D)', boxShadow:'0 8px 24px rgba(245,158,11,.3)' }}>
                  🏸
                </div>
        ...
        <div className="mx-auto flex flex-col items-center gap-5" style={{ maxWidth:460 }}>
          <div className="flex items-center gap-2">
            <img src="/logo.png" alt="matchify.pro" className="h-8 w-auto"
              style={{ filter:'drop-shadow(0 0 8px rgba(245,158,11,.4))' }} />
          ...
          <div className="rounded-2xl px-5 py-4 border text-center w-full"
            ...
            <p className="text-lg font-black mb-1.5" style={{ color:'#F59E0B' }}>PS Brothers</p>
