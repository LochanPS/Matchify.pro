    ...
    <div className="min-h-screen relative" style={{ background: 'linear-gradient(180deg,#0a0a1f 0%,#07071a 50%,#0a0a1f 100%)' }}>
      ...
      <div className="sticky top-0 z-20" style={{ background: 'rgba(7,7,26,0.95)', borderBottom: '1px solid rgba(0,255,136,0.15)', backdropFilter: 'blur(20px)' }}>
        <div className="px-4 py-3 flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-1.5">
            <div className={`w-2 h-2 rounded-full ${isConnected ? 'animate-pulse' : ''}`} style={{ background: isConnected ? '#00ff88' : '#ef4444' }} />
            <span className="text-xs font-bold" style={{ color: isConnected ? '#00ff88' : '#ef4444' }}>
              {isConnected ? 'LIVE' : 'Reconnecting...'}
            </span>
          </div>

          <button onClick={handleShare} className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-bold" style={{ background: 'rgba(0,255,136,0.15)', border: '1px solid rgba(0,255,136,0.3)', color: '#00ff88' }}>
      ...
      {shareModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(0,0,0,0.7)', backdropFilter: 'blur(8px)' }}>
          <div className="w-full max-w-sm rounded-2xl overflow-hidden" style={{ background: 'linear-gradient(135deg,#0f0f2e,#0d1117)', border: '1px solid rgba(0,255,136,0.3)' }}>
            <div className="p-6 text-center">
              <div className="w-16 h-16 mx-auto mb-4 rounded-full flex items-center justify-center" style={{ background: 'rgba(0,255,136,0.15)' }}>
...
const MatchInfo = ({ match, duration }) => {
  ...
  return (
    <div className="rounded-2xl p-4" style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(0,255,136,0.15)' }}>
      ...
      <div className="flex items-center justify-between text-xs">
        <span className="px-2.5 py-1 rounded-full font-bold" style={{
          background: match.status === 'ONGOING' ? 'rgba(0,255,136,0.15)' : 'rgba(255,255,255,0.08)',
          color: match.status === 'ONGOING' ? '#00ff88' : 'rgba(255,255,255,0.5)',
          border: `1px solid ${match.status === 'ONGOING' ? 'rgba(0,255,136,0.3)' : 'rgba(255,255,255,0.1)'}`,
        }}>
          {match.status}
        </span>
...
const Scoreboard = ({ match }) => {
  ...
  return (
    <div className="rounded-2xl p-5 relative overflow-hidden" style={{ background: 'linear-gradient(135deg,rgba(0,212,255,0.15),rgba(0,255,136,0.1))', border: '1px solid rgba(0,212,255,0.3)' }}>
...
const PlayerCard = ({ name, score, isServing, isLeading }) => (
  <div className={`text-center relative ${isLeading ? 'scale-105' : ''} transition-transform`}>
    ...
    <div className="text-5xl font-black mt-2" style={{ color: isLeading ? '#00ff88' : 'rgba(255,255,255,0.8)' }}>{score}</div>
  </div>
);
              ...
              <div key={i} className="rounded-xl p-3" style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.06)' }}>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-xs px-2 py-0.5 rounded-full font-semibold" style={{
                    background: set.winner ? 'rgba(0,255,136,0.15)' : 'rgba(0,212,255,0.15)',
                    color: set.winner ? '#00ff88' : '#00d4ff',
                  }}>
