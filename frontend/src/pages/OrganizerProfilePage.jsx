import { useState, useEffect } from 'react';
import { useParams, Link } from 'react-router-dom';
import { useAuth } from '../contexts/AuthContext';
import api from '../utils/api';
import VerifiedBadge from '../components/VerifiedBadge';
import {
  UserCircleIcon,
  TrophyIcon,
  UserGroupIcon,
  CurrencyRupeeIcon,
  StarIcon,
  MapPinIcon,
  PhoneIcon,
  EnvelopeIcon,
  CalendarIcon,
  BanknotesIcon,
  CheckCircleIcon,
  ClockIcon,
  ArrowLeftIcon
} from '@heroicons/react/24/outline';

const B = {
  bg: '#050810', card: 'rgba(255,255,255,0.04)', border: 'rgba(255,255,255,0.08)',
  cardDark: '#0d1025', green: '#06b6d4', cyan: '#00d4ff', purple: '#a855f7',
  amber: '#fbbf24', sub: 'rgba(255,255,255,0.6)', dim: 'rgba(255,255,255,0.4)',
};

export default function OrganizerProfilePage() {
  const { id } = useParams();
  const { user } = useAuth();
  const [profile, setProfile] = useState(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    fetchOrganizerProfile();
  }, [id]);

  const fetchOrganizerProfile = async () => {
    try {
      const organizerId = id || user?.id;
      const response = await api.get(`/organizer/profile/${organizerId}`);
      setProfile(response.data.profile);
    } catch (error) {
      console.error('Error fetching organizer profile:', error);
    } finally {
      setLoading(false);
    }
  };

  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ background: B.bg }}>
        <div className="text-center">
          <div className="w-16 h-16 border-4 border-t-transparent rounded-full animate-spin mx-auto"
            style={{ borderColor: 'rgba(168,85,247,0.3)', borderTopColor: B.purple }} />
          <p className="mt-4 font-medium" style={{ color: B.sub }}>Loading profile...</p>
        </div>
      </div>
    );
  }

  if (!profile) {
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ background: B.bg }}>
        <div className="text-center">
          <div className="w-20 h-20 rounded-2xl flex items-center justify-center mx-auto mb-4"
            style={{ background: 'rgba(255,255,255,0.06)' }}>
            <span className="text-4xl">❌</span>
          </div>
          <h2 className="text-2xl font-bold text-white mb-2">Profile Not Found</h2>
          <p className="mb-6" style={{ color: B.sub }}>The organizer profile you're looking for doesn't exist.</p>
          <Link
            to="/dashboard"
            className="inline-flex items-center gap-2 px-6 py-3 rounded-xl font-medium text-white transition-all"
            style={{ background: 'linear-gradient(135deg,#0891b2,#06b6d4)', color: '#050810', fontWeight: 700 }}
          >
            <ArrowLeftIcon className="w-5 h-5" />
            Back to Dashboard
          </Link>
        </div>
      </div>
    );
  }

  const isOwnProfile = !id || id === user?.id;
  const memberSince = new Date(profile.createdAt).toLocaleDateString('en-US', { month: 'long', year: 'numeric' });

  return (
    <div className="min-h-screen" style={{ background: B.bg }}>
      {/* Background orbs */}
      <div className="fixed top-0 bottom-0 pointer-events-none overflow-hidden" style={{ left: "50%", transform: "translateX(-50%)", width: "100%", maxWidth: "480px" }}>
        <div className="absolute top-1/4 left-1/4 w-96 h-96 rounded-full blur-3xl opacity-[0.06]"
          style={{ background: B.purple }} />
        <div className="absolute bottom-1/4 right-1/4 w-80 h-80 rounded-full blur-3xl opacity-[0.05]"
          style={{ background: B.cyan }} />
        <div className="absolute top-3/4 left-1/2 w-72 h-72 rounded-full blur-3xl opacity-[0.04]"
          style={{ background: B.green }} />
      </div>

      {/* Hero Header */}
      <div className="relative border-b" style={{ background: 'rgba(255,255,255,0.02)', borderColor: B.border }}>
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          <Link
            to="/dashboard"
            className="inline-flex items-center gap-2 mb-6 transition-colors"
            style={{ color: B.sub }}
          >
            <ArrowLeftIcon className="w-5 h-5" />
            Back to Dashboard
          </Link>

          <div className="flex flex-col md:flex-row items-center gap-6">
            <div className="relative">
              <div className="w-20 h-20 sm:w-32 sm:h-32 rounded-2xl flex items-center justify-center text-3xl sm:text-5xl font-bold text-white shadow-2xl overflow-hidden"
                style={{ background: 'linear-gradient(135deg,#a855f7,#7c3aed)', boxShadow: '0 8px 30px rgba(168,85,247,0.35)' }}>
                {profile.profilePhoto ? (
                  <img src={profile.profilePhoto} alt={profile.name} className="w-full h-full object-cover" />
                ) : (
                  profile.name?.charAt(0)?.toUpperCase() || 'O'
                )}
              </div>
              {profile.isVerifiedOrganizer && (
                <div className="absolute -bottom-2 -right-2">
                  <VerifiedBadge type="organizer" size="xl" />
                </div>
              )}
            </div>

            <div className="flex-1 text-center md:text-left">
              <div className="flex items-center gap-3 justify-center md:justify-start mb-2">
                <h1 className="text-2xl sm:text-4xl font-bold text-white">{profile.name}</h1>
                {profile.isVerifiedOrganizer && (
                  <VerifiedBadge type="organizer" size="lg" showText />
                )}
              </div>

              {profile.organizerProfile?.organization && (
                <p className="text-xl mb-3" style={{ color: 'rgba(255,255,255,0.8)' }}>{profile.organizerProfile.organization}</p>
              )}

              <div className="flex flex-wrap items-center gap-4 text-sm justify-center md:justify-start" style={{ color: B.sub }}>
                {profile.email && (
                  <span className="flex items-center gap-1">
                    <EnvelopeIcon className="w-4 h-4" />
                    {profile.email}
                  </span>
                )}
                {profile.phone && (
                  <span className="flex items-center gap-1">
                    <PhoneIcon className="w-4 h-4" />
                    +91 {profile.phone}
                  </span>
                )}
                {(profile.city || profile.state) && (
                  <span className="flex items-center gap-1">
                    <MapPinIcon className="w-4 h-4" />
                    {[profile.city, profile.state].filter(Boolean).join(', ')}
                  </span>
                )}
                <span className="flex items-center gap-1">
                  <CalendarIcon className="w-4 h-4" />
                  Member since {memberSince}
                </span>
              </div>
            </div>
          </div>
        </div>
      </div>

      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {/* Stats Cards */}
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          <div className="rounded-2xl p-6 border transition-all" style={{ background: B.card, borderColor: B.border }}>
            <div className="inline-flex items-center justify-center w-12 h-12 rounded-xl shadow-lg mb-4"
              style={{ background: 'linear-gradient(135deg,#00b4d8,#00d4ff)' }}>
              <TrophyIcon className="w-6 h-6 text-white" />
            </div>
            <p className="text-3xl font-bold text-white">{profile.stats.tournamentsOrganized}</p>
            <p className="text-sm mt-1" style={{ color: B.dim }}>Tournaments Organized</p>
          </div>

          <div className="rounded-2xl p-6 border transition-all" style={{ background: B.card, borderColor: B.border }}>
            <div className="inline-flex items-center justify-center w-12 h-12 rounded-xl shadow-lg mb-4"
              style={{ background: 'linear-gradient(135deg,#a855f7,#7c3aed)' }}>
              <UserGroupIcon className="w-6 h-6 text-white" />
            </div>
            <p className="text-3xl font-bold text-white">{profile.stats.totalParticipants}</p>
            <p className="text-sm mt-1" style={{ color: B.dim }}>Total Participants</p>
          </div>

          <div className="rounded-2xl p-6 border transition-all" style={{ background: B.card, borderColor: B.border }}>
            <div className="inline-flex items-center justify-center w-12 h-12 rounded-xl shadow-lg mb-4"
              style={{ background: 'linear-gradient(135deg,#0891b2,#06b6d4)' }}>
              <CurrencyRupeeIcon className="w-6 h-6 text-white" />
            </div>
            <p className="text-3xl font-bold text-white">₹{profile.stats.totalRevenue}</p>
            <p className="text-sm mt-1" style={{ color: B.dim }}>Total Revenue</p>
          </div>

          <div className="rounded-2xl p-6 border transition-all" style={{ background: B.card, borderColor: B.border }}>
            <div className="inline-flex items-center justify-center w-12 h-12 rounded-xl shadow-lg mb-4"
              style={{ background: 'linear-gradient(135deg,#f59e0b,#fbbf24)' }}>
              <StarIcon className="w-6 h-6 text-white" />
            </div>
            <p className="text-3xl font-bold text-white">{profile.organizerProfile?.rating || 'N/A'}</p>
            <p className="text-sm mt-1" style={{ color: B.dim }}>Rating</p>
          </div>
        </div>

        <div className="grid lg:grid-cols-2 gap-6">
          {/* Profile Information */}
          <div className="rounded-2xl p-6 border" style={{ background: B.card, borderColor: B.border }}>
            <div className="flex items-center gap-3 mb-6">
              <div className="w-10 h-10 rounded-xl flex items-center justify-center"
                style={{ background: 'linear-gradient(135deg,#a855f7,#7c3aed)' }}>
                <UserCircleIcon className="w-5 h-5 text-white" />
              </div>
              <h3 className="text-lg font-bold text-white">Profile Information</h3>
            </div>

            <div className="space-y-4">
              <div>
                <p className="text-xs mb-1" style={{ color: B.dim }}>Full Name</p>
                <p className="text-white font-medium">{profile.name}</p>
              </div>

              {profile.organizerProfile?.organization && (
                <div>
                  <p className="text-xs mb-1" style={{ color: B.dim }}>Organization</p>
                  <p className="text-white font-medium">{profile.organizerProfile.organization}</p>
                </div>
              )}

              {profile.email && (
                <div>
                  <p className="text-xs mb-1" style={{ color: B.dim }}>Email Address</p>
                  <p className="text-white font-medium text-sm break-all">{profile.email}</p>
                </div>
              )}

              {profile.phone && (
                <div>
                  <p className="text-xs mb-1" style={{ color: B.dim }}>Phone Number</p>
                  <p className="text-white font-medium">{profile.phone}</p>
                </div>
              )}

              {(profile.city || profile.state || profile.country) && (
                <div>
                  <p className="text-xs mb-1" style={{ color: B.dim }}>Location</p>
                  <p className="text-white font-medium">
                    {[profile.city, profile.state, profile.country].filter(Boolean).join(', ')}
                  </p>
                </div>
              )}

              <div>
                <p className="text-xs mb-1" style={{ color: B.dim }}>Member Since</p>
                <p className="text-white font-medium">{memberSince}</p>
              </div>

              <div>
                <p className="text-xs mb-1" style={{ color: B.dim }}>Verification Status</p>
                {profile.isVerifiedOrganizer ? (
                  <div className="flex items-center gap-2">
                    <CheckCircleIcon className="w-5 h-5" style={{ color: B.green }} />
                    <span className="font-medium" style={{ color: B.green }}>Verified Organizer</span>
                  </div>
                ) : (
                  <div className="flex items-center gap-2">
                    <ClockIcon className="w-5 h-5" style={{ color: B.amber }} />
                    <span className="font-medium" style={{ color: B.amber }}>Not Verified</span>
                  </div>
                )}
              </div>
            </div>
          </div>

          {/* Financial Information */}
          <div className="rounded-2xl p-6 border" style={{ background: B.card, borderColor: B.border }}>
            <div className="flex items-center gap-3 mb-6">
              <div className="w-10 h-10 rounded-xl flex items-center justify-center"
                style={{ background: 'linear-gradient(135deg,#0891b2,#06b6d4)' }}>
                <BanknotesIcon className="w-5 h-5 text-white" />
              </div>
              <h3 className="text-lg font-bold text-white">Financial Overview</h3>
            </div>

            <div className="space-y-6">
              <div className="rounded-xl p-4" style={{ background: 'rgba(0,0,0,0.2)' }}>
                <p className="text-sm mb-2" style={{ color: B.sub }}>Total Revenue</p>
                <p className="text-3xl font-bold" style={{ color: B.green }}>₹{profile.stats.totalRevenue}</p>
                <p className="text-xs mt-1" style={{ color: B.dim }}>From all tournaments</p>
              </div>

              <div className="rounded-xl p-4" style={{ background: 'rgba(0,0,0,0.2)' }}>
                <p className="text-sm mb-2" style={{ color: B.sub }}>Paid Out</p>
                <p className="text-2xl font-bold" style={{ color: B.green }}>₹{profile.stats.paidOut}</p>
                <p className="text-xs mt-1" style={{ color: B.dim }}>Successfully transferred</p>
              </div>

              <div className="rounded-xl p-4" style={{ background: 'rgba(0,0,0,0.2)' }}>
                <p className="text-sm mb-2" style={{ color: B.sub }}>Pending Payout</p>
                <p className="text-2xl font-bold" style={{ color: B.amber }}>₹{profile.stats.pendingPayout}</p>
                <p className="text-xs mt-1" style={{ color: B.dim }}>Awaiting transfer</p>
              </div>

              {isOwnProfile && profile.organizerProfile?.savedUpiId && (
                <div className="rounded-xl p-4 border"
                  style={{ background: 'rgba(0,212,255,0.06)', borderColor: 'rgba(0,212,255,0.25)' }}>
                  <p className="text-sm font-medium mb-2" style={{ color: B.cyan }}>Saved Payment Details</p>
                  <div className="space-y-2">
                    <div>
                      <p className="text-xs" style={{ color: B.dim }}>UPI ID</p>
                      <p className="text-white text-sm font-mono">{profile.organizerProfile.savedUpiId}</p>
                    </div>
                    {profile.organizerProfile.savedAccountHolder && (
                      <div>
                        <p className="text-xs" style={{ color: B.dim }}>Account Holder</p>
                        <p className="text-white text-sm">{profile.organizerProfile.savedAccountHolder}</p>
                      </div>
                    )}
                  </div>
                </div>
              )}
            </div>
          </div>
        </div>

        {/* Quick Actions */}
        {isOwnProfile && (
          <div className="mt-8 rounded-2xl p-6 border" style={{ background: B.card, borderColor: B.border }}>
            <h3 className="text-lg font-bold text-white mb-4">Quick Actions</h3>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <Link
                to="/tournaments/create"
                className="flex items-center gap-4 p-4 rounded-xl border transition-all group"
                style={{ background: 'rgba(0,0,0,0.2)', borderColor: 'rgba(255,255,255,0.06)' }}
              >
                <div className="w-12 h-12 rounded-xl flex items-center justify-center group-hover:scale-110 transition-transform"
                  style={{ background: 'linear-gradient(135deg,#00b4d8,#00d4ff)' }}>
                  <TrophyIcon className="w-6 h-6 text-white" />
                </div>
                <div>
                  <p className="font-semibold text-white">Create Tournament</p>
                  <p className="text-sm" style={{ color: B.dim }}>Start organizing</p>
                </div>
              </Link>

              <Link
                to="/organizer/history"
                className="flex items-center gap-4 p-4 rounded-xl border transition-all group"
                style={{ background: 'rgba(0,0,0,0.2)', borderColor: 'rgba(255,255,255,0.06)' }}
              >
                <div className="w-12 h-12 rounded-xl flex items-center justify-center group-hover:scale-110 transition-transform"
                  style={{ background: 'linear-gradient(135deg,#0891b2,#06b6d4)' }}>
                  <CalendarIcon className="w-6 h-6 text-white" />
                </div>
                <div>
                  <p className="font-semibold text-white">View History</p>
                  <p className="text-sm" style={{ color: B.dim }}>Past tournaments</p>
                </div>
              </Link>

              <Link
                to="/profile"
                className="flex items-center gap-4 p-4 rounded-xl border transition-all group"
                style={{ background: 'rgba(0,0,0,0.2)', borderColor: 'rgba(255,255,255,0.06)' }}
              >
                <div className="w-12 h-12 rounded-xl flex items-center justify-center group-hover:scale-110 transition-transform"
                  style={{ background: 'linear-gradient(135deg,#a855f7,#7c3aed)' }}>
                  <UserCircleIcon className="w-6 h-6 text-white" />
                </div>
                <div>
                  <p className="font-semibold text-white">Edit Profile</p>
                  <p className="text-sm" style={{ color: B.dim }}>Update details</p>
                </div>
              </Link>
            </div>
          </div>
        )}
      </div>
    </div>
  );
}
