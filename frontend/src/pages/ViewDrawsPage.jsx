  ...
  return (
    <div className="min-h-screen" style={{ background: '#07071a' }}>
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {success && (
          <div className="mb-6 rounded-xl p-4 flex items-center gap-3" style={{ background: 'rgba(0,255,136,0.08)', border: '1px solid rgba(0,255,136,0.3)' }}>
            <CheckCircle className="w-5 h-5 flex-shrink-0" style={{ color: '#00ff88' }} />
            <span className="font-medium" style={{ color: '#00ff88' }}>{success}</span>
            <button onClick={() => setSuccess(null)} className="ml-auto" style={{ color: '#00ff88' }}><X className="w-5 h-5" /></button>
          </div>
        )}
        ...
        {categories.length === 0 ? (
          <div className="bg-slate-800/50 backdrop-blur-sm border border-white/10 rounded-2xl p-12 text-center">
            ...
            {isOrganizer && (
              <button onClick={() => navigate(`/tournaments/${id}/categories`)} className="px-6 py-3 text-white rounded-xl hover:shadow-lg transition-all font-semibold" style={{ background: 'linear-gradient(135deg,#00ff88,#00d4ff)' }}>
                Manage Categories
              </button>
            )}
          </div>
                  ...
                  <div className="px-6 py-4 border-b border-white/10 flex items-center justify-between">
                    ...
                    {isOrganizer && (
                      <button
                        ...
                        className="flex items-center gap-2 px-4 py-2 text-white rounded-xl hover:shadow-lg transition-all font-semibold" style={{ background: 'linear-gradient(135deg,#00ff88,#00d4ff)' }}
                      >
                  ...
                  <div className="p-6">
                    {draw ? (
                      ...
                      <div className="text-center py-12">
                        <div className="w-20 h-20 rounded-2xl flex items-center justify-center mx-auto mb-4" style={{ background: 'rgba(0,255,136,0.08)' }}>
                          <GitBranch className="w-10 h-10" style={{ color: '#00ff88' }} />
                        </div>
                        ...
                        {isOrganizer && (
                          <button
                            onClick={() => setShowConfigModal(true)}
                            className="px-6 py-3 text-white rounded-xl hover:shadow-lg transition-all font-semibold" style={{ background: 'linear-gradient(135deg,#00ff88,#00d4ff)' }}
                          >
...
const DrawConfigModal = ({ category, existingDraw, onClose, onSave, saving }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="border border-white/10 rounded-2xl max-w-lg w-full max-h-[90vh] overflow-y-auto" style={{ background: '#0d1025' }}>
        ...
        <div className="p-6 space-y-6">
            ...
            <div className="space-y-3">
              {formatOptions.map((option) => (
                <button
                  ...
                  style={config.format === option.value
          ...
          <div>
            ...
            <select
              ...
              className="w-full px-4 py-3 border rounded-xl text-white focus:outline-none" style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)' }}
            >
                  ...
                  {groupOptions.map(num => (
                    <button
                      ...
                      style={config.numberOfGroups === num
                        ? { background: '#00ff88', color: '#07071a' }
        ...
        <div className="p-6 border-t border-white/10 flex gap-3">
          ...
          <button
            onClick={handleSave}
            disabled={saving}
            className="flex-1 px-4 py-3 text-white rounded-xl hover:shadow-lg transition-all font-semibold disabled:opacity-50" style={{ background: 'linear-gradient(135deg,#00ff88,#00d4ff)', color: '#07071a' }}
          >
...
const DrawBracket = ({ draw, categoryFormat }) => {
  ...
  if (format === 'ROUND_ROBIN') {
    return <RoundRobinDraw data={data} categoryFormat={categoryFormat} />;
  }
  if (format === 'ROUND_ROBIN_KNOCKOUT') {
    return <GroupsKnockoutDraw data={data} categoryFormat={categoryFormat} />;
  }
  return <KnockoutBracket data={data} categoryFormat={categoryFormat} />;
};

// Knockout Bracket Component
const KnockoutBracket = ({ data, categoryFormat }) => {
  ...
  const getPlayerDisplay = (player) => {
    if (!player || !player.id || !player.name || player.name === 'TBD') return 'TBD';
  ...
  // Determine card width based on category format
  const isDoubles = categoryFormat === 'doubles';
  const cardWidth = isDoubles ? 'w-[360px]' : 'w-[300px]';
  const gapSize = isDoubles ? 'gap-20' : 'gap-16';

  return (
    <div className="overflow-x-auto py-6">
      <div className={`flex ${gapSize} min-w-max px-8`}>
        {data.rounds.map((round, ri) => (
          <div key={ri} className="flex flex-col">
            {/* Round Title - Subtle but clear */}
            <div className="mb-6">
              <h4 className="text-sm font-semibold text-gray-400 text-center tracking-wider uppercase">
                {getRoundName(ri, data.rounds.length)}
              </h4>
            </div>
            
            <div className="flex flex-col justify-around flex-1 gap-10" style={{ paddingTop: ri > 0 ? `${Math.pow(2, ri) * 32}px` : 0 }}>
              {round.matches.map((match, mi) => {
                const isPlayer1Winner = match.winner === 1 || match.winnerId === match.player1?.id;
                const isPlayer2Winner = match.winner === 2 || match.winnerId === match.player2?.id;
                const hasWinner = isPlayer1Winner || isPlayer2Winner;
                
                return (
                  <div 
                    key={mi} 
                    className={`${cardWidth} bg-slate-800/60 backdrop-blur-sm rounded-lg border border-slate-700/50 shadow-lg hover:border-slate-600/50 transition-all duration-200`}
                    style={{ marginBottom: ri > 0 ? `${Math.pow(2, ri) * 64}px` : 0 }}
                  >
                    ...
                    <div className="px-4 py-2.5 border-b border-slate-700/50 flex items-center justify-between">
                      ...
                      {hasWinner && (
                        <span className="text-[10px] font-semibold tracking-wider uppercase" style={{ color: '#00ff88' }}>
                          Completed
                        </span>
                      )}
                    </div>

                    {/* Players Container - Optimized spacing */}
                    <div className="px-4 py-4 space-y-2">
                      {/* Player 1 */}
                      <div className="flex items-center justify-between py-2.5 px-3 rounded-md transition-colors"
                        ...
                        <div className="flex items-center gap-2 flex-1 min-w-0">
                          ...
                          <span className={`text-sm font-semibold truncate ${
                            isPlayer1Winner ? 'text-white' : 'text-gray-300'
                          }`}>
                      ...
                      <div className="flex items-center justify-center py-1">
                        <span className="text-[10px] font-medium text-gray-600 tracking-widest">VS</span>
                      </div>
                      ...
                      <div className="flex items-center justify-between py-2.5 px-3 rounded-md transition-colors"
                        ...
                        <div className="flex items-center gap-2 flex-1 min-w-0">
                          ...
                          <span className={`text-sm font-semibold truncate ${
                            isPlayer2Winner ? 'text-white' : 'text-gray-300'
                          }`}>
...
const RoundRobinDraw = ({ data, categoryFormat }) => {
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-white/10">
                  ...
                  <th className="text-left py-2 px-3 text-gray-400">{categoryFormat === 'doubles' ? 'Team' : 'Player'}</th>
              ...
              <tbody>
                {group.participants.map((p, pi) => (
                  <tr key={pi} className="border-b border-white/5">
                    ...
                    <td className="py-2 px-3 text-center" style={{ color: '#00ff88' }}>{p.wins || 0}</td>
...
const GroupsKnockoutDraw = ({ data, categoryFormat }) => {
  return (
    <div className="space-y-8">
      {/* Group Stage */}
      <div>
        <h3 className="text-lg font-bold text-white mb-4 flex items-center gap-2">
          <span className="px-3 py-1 rounded-lg text-sm" style={{ background: 'rgba(0,255,136,0.1)', color: '#00ff88' }}>Stage 1</span>
      ...
      {data.knockout && (
        <div>
          <h3 className="text-lg font-bold text-white mb-4 flex items-center gap-2">
            <span className="px-3 py-1 rounded-lg text-sm" style={{ background: 'rgba(0,255,136,0.1)', color: '#00ff88' }}>Stage 2</span>
            Knockout Stage
          </h3>
